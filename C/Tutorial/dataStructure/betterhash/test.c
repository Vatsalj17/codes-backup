...
int main(int argc, char** argv) {
	if (argc != 3) {
		printf("Usage: %s <wordlist filepath> <numguesses>\n", argv[0]);
		return EXIT_FAILURE;
	}
	char* filename = argv[1];
	uint32_t num_guesses = atol(argv[2]);
	int tablesize = (1 << 20);	// 2^20
